...
modes = ["OR_inertia", "inertia", "inertia_noSyn"]
...
for mode in modes:
    for region in regions: #looping through regions and modes in this way means that it will first run all "pre", then all "OR" and so on, instead of first solving all modes for each region
        ...
        scenarios[region+"_"+mode+"_lowROCOF"] =  \
        f"""
$setglobal tot_opt "{region}_{mode}{"_3h" if "_3h" in mode else ""}_lowROCOF" 
...
$setglobal sync_cond_price {"3" if "_3xCost" in mode else "1"}
$setglobal bat_price 1
...
$setglobal hour_resolution {3 if "_3h" in mode else 1}"""
#[THE ABOVE NEEDS TO BE CONFIGURED WHEN SETTING SCRIPT UP]

num_theads = min(5, len(scenarios)) # should probably be equal or half of your nr of cores (or nr of scenarios, whichever is lower)
...
def run_scenario(workspace, checkpoint, io_lock, scen):
    ...
    print(f" --- Starting scenario {scen[0]}: {scen[1]} in thread",thread_nr[threading.get_ident()],"at",dt.datetime.now().strftime('%H:%M:%S'),"---")
