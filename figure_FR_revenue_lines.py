...
technologies_to_show = ["bat","PtH","thermals","Hydro", "H2store"]
per_cap = True
fig, axs = plt.subplots(nrows=len(technologies_to_show), ncols=1, figsize=(6, 7))
...
for region in regions:
    ...
    temp_df = df_share[flexes[0]].loc[region].astype(str)
    for r in range(temp_df.shape[0]):
        for c in range(temp_df.shape[1]):
            lf_val = df_share[flexes[0]].loc[region].iat[r, c]
            hf_val = df_share[flexes[1]].loc[region].iat[r, c]
            try: temp_df.iat[r, c] = f"{round(lf_val)} / {round(hf_val)}"
            except:
                print_red(r,c)
                print_red(temp_df)
                raise
    print_green('\n'.join(temp_df.astype(str).apply(lambda x: ', '.join(x), axis=1)))
print(temp_df)
        # Plotting the data
for i_t, tech in enumerate(technologies_to_show):
    ...
    for i_r, region in enumerate(regions):
        axs[i_t].fill_between(years,df_rev[tech].loc[(region,"lowFlex")],df_rev[tech].loc[(region,"highFlex")],
                              alpha=0.3, color=['teal', 'purple', 'orange'][i_r])
...
plt.figtext(0,0.5,f"Revenue{' per capacity' if per_cap else ''} [M€/{'GW(h)/' if per_cap else ''}yr]", fontsize=12, rotation=90, va="center",ha="center")
...
legend = fig.legend(lines, ["Nordic+","Brit","Iberia"], loc="lower center", bbox_transform=axs[0].transAxes, bbox_to_anchor=(0.5, 1.29), ncol=3)
fig.suptitle(f"Reserve supply revenue{' per installed capacity' if per_cap else ''}", y=1.017, fontsize=13)
plt.tight_layout()
fig_path = f"figures\\"
